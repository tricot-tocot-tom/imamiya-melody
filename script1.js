...
const APP_VERSION='v4.5 JSON救出版';
...
function renderCover(){
  ...
  if(pool.length){
    ...
    img.onerror=()=>{meta.textContent='Poco画像が見つかりません。poco-editor.pngも一緒にアップしてください。';};
...
function collectStateCandidates(obj, out=[], path='root', depth=0){
  if(!obj || depth>5) return out;
  obj=parseMaybeJson(obj);
  if(!obj || typeof obj!=='object') return out;
  if(Array.isArray(obj)){
    obj.forEach((v,i)=>collectStateCandidates(v,out,`${path}[${i}]`,depth+1));
    return out;
  }
  if(Array.isArray(obj.photos)) out.push({path,obj});
  const likelyKeys=['state','data','backup','payload','current','appState','imamiya_melody_henshubu_v2','IMAMIYA_MELODY','IMAMIYA_Melody'];
  for(const k of likelyKeys){ if(obj[k]!=null) collectStateCandidates(obj[k],out,`${path}.${k}`,depth+1); }
  if(obj.localStorage && typeof obj.localStorage==='object'){
    for(const [k,v] of Object.entries(obj.localStorage)) collectStateCandidates(v,out,`${path}.localStorage.${k}`,depth+1);
  }
  if(Array.isArray(obj.items)){
    obj.items.forEach((it,i)=>{
      if(it && typeof it==='object'){
        collectStateCandidates(it.value ?? it.data ?? it.state ?? it, out, `${path}.items[${i}]`, depth+1);
      }
    });
  }
  for(const [k,v] of Object.entries(obj)){
    if(/imamiya|melody|henshubu|photo|storage|rescue|backup/i.test(k)){
      collectStateCandidates(v,out,`${path}.${k}`,depth+1);
    }
  }
  return out;
}
function normalizePhoto(p,i=0){
  ...
  const name=p.name||p.fileName||p.filename||p.title||p.id||`photo-${i+1}`;
  const src=p.src||p.thumb||p.thumbnail||p.thumbnailUrl||p.dataUrl||p.dataURL||p.url||p.image||p.imageData||p.base64||'';
  const finalSrc=(typeof src==='string' && src && !src.startsWith('data:') && /^[A-Za-z0-9+/=]+$/.test(src.slice(0,60)) && src.length>200) ? 'data:image/jpeg;base64,'+src : src;
  return {id:p.id||safeId(),name:String(name),src:finalSrc,camera:p.camera||p.cam||guessCamera(String(name)),rating:+(p.rating||p.star||p.stars||0),reason:p.reason||p.memo||p.note||'',instruction:p.instruction||p.editInstruction||'',placement:p.placement||'未定',sticky:p.sticky||p.tag||'',created:p.created||p.createdAt||p.date||new Date().toISOString(),seen:!!(p.seen||p.viewed)};
}
function normalizeImportedState(obj){
  ...
  const candidates=collectStateCandidates(obj);
  let source=obj;
  if(candidates.length){
    candidates.sort((a,b)=>(b.obj.photos?.length||0)-(a.obj.photos?.length||0));
    source=candidates[0].obj;
  }else if(Array.isArray(obj)){
    source={photos:obj};
  }
  let next={...base,...source};
  if(!Array.isArray(next.photos)){
    const possible=[];
    for(const [k,v] of Object.entries(source||{})){
      if(Array.isArray(v) && /photo|image|receipt|items|files/i.test(k)) possible.push(...v);
    }
    next.photos=possible;
  }
  ...
  next.photos=(next.photos||[]).map(normalizePhoto).filter(p=>p && p.src);
  cameras.forEach(c=>{if(!next.receipts[c]) next.receipts[c]={status:'準備中',count:next.photos.filter(p=>p.camera===c).length,last:''};});
  next.__candidateCount=candidates.length;
  return next;
}
